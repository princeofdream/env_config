...
if [ ! -r "./libs/std" ]
then
tar -C ./libs -xzf ./libs/linux64_std.tar.gz
fi
...
DONTNEEDEX
    ...
    then
        cp -fr ./libs/std/libtiff.so.3 ./libs/libtiff.so.3
    fi
    if [ ! -r "/usr/lib64/libtiff.so.3.9.4" ]
    then
        cp -fr ./libs/std/libtiff.so.3.9.4 ./libs/libtiff.so.3.9.4
    fi
    if [ ! -r "/usr/lib64/libstdc++.so.6" ]
    then
        cp -fr ./libs/std/libstdc++.so.6 ./libs/libstdc++.so.6
    fi
    if [ ! -r "/usr/lib64/libstdc++.so.6.0.13" ]
    then
        cp -fr ./libs/std/libstdc++.so.6.0.13 ./libs/libstdc++.so.6.0.13
    fi
...
then 
    ...
    then
        cp -fr ./libs/std/libpangox-1.0.so.0 ./libs/libpangox-1.0.so.0
    fi
    
    if [ ! -r "/usr/lib64/libpng12.so" ]
    then
        cp -fr ./libs/std/libpng12.so ./libs/libpng12.so
    fi
    if [ ! -r "/usr/lib64/libpng12.so.0" ]
    then
        cp -fr ./libs/std/libpng12.so.0 ./libs/libpng12.so.0
    fi
    if [ ! -r "/usr/lib64/libpng12.so.0.44.0" ]
    then
        cp -fr ./libs/std/libpng12.so.0.44.0 ./libs/libpng12.so.0.44.0
    fi
...
DONTNEEDEX
	...
	then
	cp -fr ./libs/std/libstdc++.so.6 ./libs/
	fi
...
then
	...
	if [ ! -r "/usr/lib/libpng12.so" ]
    then
        cp -rf ./libs/std/libpng12.* ./libs/
    fi
	
	if [ ! -r "/usr/lib/libjpeg.so" ]
    then
        cp -rf ./libs/std/libjpeg.* ./libs/
    fi
...
then
    ...
    then
        cp -fr ./libs/std/libtiff.so.3 ./libs/libtiff.so.3
        #cp -fr ./libs/std/libtiff.so.3 ./libs/x86_64-linux-gnu/libtiff.so.3  
    fi

#   if [ ! -r "/usr/lib/x86_64-linux-gnu/libjpeg.so.62" ] && [ ! -r "/usr/lib/libjpeg.so.62" ]
#   then
#        ln -s /usr/lib/x86_64-linux-gnu/libjpeg.so.8.0.2 ./libs/libjpeg.so.62
#   fi
...
#    rm -f /usr/lib/libjpeg.so.62
    cp -frH ./libs/std/libjpeg.so.62 ./libs/ > /dev/null 2>&1
...
chcon -t textrel_shlib_t ./libs/libCoreUtils.so
chcon -t textrel_shlib_t ./libs/libImplAv.so
chcon -t textrel_shlib_t ./libs/libOesisCore.so
fi
